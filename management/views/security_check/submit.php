...
if (isset($_POST['submit'])) {
    if ($_POST['submit'] == 'accept') {
        $conn->query("UPDATE `ticket` SET `stage` = 'immigration' WHERE `id` = '$id'");
    } else if ($_POST['submit'] == 'reject') {
        $conn->query("UPDATE `ticket` SET `stage` = 'rejected_security_check' WHERE `id` = '$id'");
    }
}
...
<div class="container">
    <div class="d-flex align-items-center justify-content-between">
        ...
        <p class="h3 my-3 font-weight-light">Security Check</p>
    </div>
    <hr>
    <?php if ($ticket['stage'] == 'security_check') { ?>
        <div class="row">
            <div class="col-md-8">
                <div class="card mb-3">
                    <div class="card-body">
        ...
        <div class="card">
            <div class="card-body">
                <form action="" method="post" class="d-flex align-items-center justify-content-between">
                    ...
                    <button name="submit" value="accept" class="btn btn-primary"><i class="fa fa-check"></i> Verify</button>
                </form>
            </div>
        </div>
    <?php } else { ?>
